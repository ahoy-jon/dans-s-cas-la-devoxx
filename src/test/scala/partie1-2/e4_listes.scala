...
class e4_listes extends HandsOnSuite {
  ...
  */
    ...
    // Nothing <: Student <: Person

    val nil:List[Nothing] = Nil
    val listStudent = List[Student](Student("yo"))
    val listPerson:List[Person] = Nil


    listStudent.map(student => if(student.name == "yo") student.copy("yolo") else student)
